...
#import "VSMHTTPRequestPrivate.h"
#import "VSMHTTPRequestContext.h"
#import "NSObject+VSMUtils.h"

const NSTimeInterval kVSMHTTPRequestDefaultTimeout = 45.0f;
NSString *const kVSMHTTPRequestDefaultMethod = @"POST";
NSString *const kVSMHTTPRequestErrorDomain = @"VSMHTTPRequestErrorDomain";

NSString *const kVSMAccessTokenHeader = @"Authorization";

@interface VSMHTTPRequest ()

+ (NSString * __nonnull)HTTPMethodNameForMethod:(HTTPRequestMethod)method;

@end

@implementation VSMHTTPRequest
...
- (NSURLRequest *)request {
    ...
    r.timeoutInterval = kVSMHTTPRequestDefaultTimeout;
    r.HTTPMethod = kVSMHTTPRequestDefaultMethod;
    _request = r;
    return _request;
}

- (VSMHTTPRequestCompletionHandler)completionHandler {
    VSMHTTPRequestCompletionHandler blk = nil;
    @synchronized(self) {
...
- (void)setCompletionHandler:(VSMHTTPRequestCompletionHandler)completionHandler {
    @synchronized(self) {
...
- (instancetype)initWithContext:(VSMHTTPRequestContext *)context; {
    self = [super init];
    if (self == nil) {
        return nil;
    }
...
- (instancetype)init {
    return [self initWithContext:[[VSMHTTPRequestContext alloc] init]];
}
...
- (NSError *)handleError:(NSObject *)candidate code:(NSInteger)code {
    return [candidate vsm_as:[NSError class]];
}
...
- (void)setRequestMethod:(HTTPRequestMethod)method {
    NSMutableURLRequest *r = [self.request vsm_as:[NSMutableURLRequest class]];
    r.HTTPMethod = [[self class] HTTPMethodNameForMethod:method];
}

- (void)setRequestHeaders:(NSDictionary *)headers {
    for (NSString *header in headers.allKeys) {
        NSString *value = [headers[header] vsm_as:[NSString class]];
        if (value != nil) {
            NSMutableURLRequest *r = [self.request vsm_as:[NSMutableURLRequest class]];
...
- (NSURLSessionDataTask * __nonnull)taskForSession:(NSURLSession * __nonnull)session {
    NSURLSessionDataTask *task = [session dataTaskWithRequest:self.request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (self.completionHandler != nil) {
            __weak VSMHTTPRequest *weakRequest = self;
            if (error != nil) {
                self.error = error;
            ...
            NSHTTPURLResponse *httpResponse = [response vsm_as:[NSHTTPURLResponse class]];
#if USE_SERVICE_REQUEST_DEBUG
            VSMRDLog(@"%@: response URL: %@", NSStringFromClass(self.class), httpResponse.URL);
            VSMRDLog(@"%@: response HTTP status code: %ld", NSStringFromClass(self.class), (long)httpResponse.statusCode);
            VSMRDLog(@"%@: response headers: %@", NSStringFromClass(self.class), httpResponse.allHeaderFields);
            if( data.length != 0 )
            {
                NSString *bodyStr = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
                VSMRDLog(@"%@: response body: %@", NSStringFromClass(self.class), bodyStr);
            }
#endif
            ...
            switch (httpResponse.statusCode) {
                case 200:
                    break;
                default:
                    self.error = [[NSError alloc] initWithDomain:kVSMHTTPRequestErrorDomain code:httpResponse.statusCode userInfo:nil];
                    break;
            }
...
+ (NSString *)HTTPMethodNameForMethod:(HTTPRequestMethod)method {
    ...
    switch (method) {
        ...
        case DELETE:
            ...
            break;
        default:
            name = kVSMHTTPRequestDefaultMethod;
            break;
    }
    return name;
}
